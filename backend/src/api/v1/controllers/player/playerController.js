...
const createPlayer = async (req, res, next) => {
  ...
  try {
    ...
    await logActivity(req, "Thêm cầu thủ", `Đã thêm cầu thủ "${savedPlayer.name}" (Số áo: ${savedPlayer.number}, Vị trí: ${savedPlayer.position}) vào đội bóng "${team.team_name}"`);
...
const updatePlayer = async (req, res, next) => {
  ...
  try {
    ...
    if (updates.number !== undefined && updates.number.trim() !== "" && updates.number !== player.number) {
    ...
    await logActivity(req, "Cập nhật cầu thủ", `Đã cập nhật thông tin cầu thủ "${updatedPlayer.name}" (Đội bóng: ${team.team_name})`);
...
const deletePlayer = async (req, res, next) => {
  ...
  try {
    ...
    await logActivity(req, "Xóa cầu thủ", `Đã xóa cầu thủ "${player.name}" khỏi hệ thống`);
