...
export const createOrUpdateLineup = async (req, res, next) => {
  ...
  try {
    ...
    if (players.length !== 11) {
        throw Object.assign(new Error(`Đội hình ra sân phải có đúng 11 cầu thủ.`), { status: 400 });
    }

    const gkCount = players.filter(p => p.position === 'GK' || p.position === 'Thủ môn').length;
    if (gkCount !== 1) {
        throw Object.assign(new Error(`Đội hình phải có chính xác 1 thủ môn (GK).`), { status: 400 });
    }
