...
const createSeason = async (req, res, next) => {
  ...
  try {
    ...
    await logActivity(req, "Tạo mùa giải", `Đã tạo mùa giải mới "${savedSeason.season_name}" (Từ ngày ${new Date(savedSeason.start_date).toLocaleDateString('vi-VN')} đến ${new Date(savedSeason.end_date).toLocaleDateString('vi-VN')})`);
...
const updateSeason = async (req, res, next) => {
  ...
  try {
    ...
    await logActivity(req, "Cập nhật mùa giải", `Đã cập nhật thông tin mùa giải "${updatedSeason.season_name}"`);
...
const deleteSeason = async (req, res, next) => {
  ...
  try {
    ...
    await logActivity(req, "Xóa mùa giải", `Đã xóa mùa giải "${seasonToDelete.season_name}" và toàn bộ dữ liệu liên quan`);
