...
const getTeams = async (req, res, next) => {
  try {
    const teams = await Team.find().populate("season_id").lean();
    for (let team of teams) {
      team.playerCount = await Player.countDocuments({ team_id: team._id });
    }
...
const createTeam = async (req, res, next) => {
  ...
  try {
    ...
    const newTeam = new Team({
      ...
      team_name,
      stadium,
      coach: coach || 'Chưa cập nhật',
      logo: logo || 'https://upload.wikimedia.org/wikipedia/commons/thumb/a/ac/No_image_available.svg/300px-No_image_available.svg.png',
    });
    const savedTeam = await newTeam.save({ session });

    await logActivity(req, "Tạo đội bóng", `Đã tạo đội bóng "${savedTeam.team_name}" (Sân nhà: ${savedTeam.stadium}, HLV: ${savedTeam.coach})`);
...
const updateTeam = async (req, res, next) => {
  ...
  try {
    ...
    if (team_name !== undefined) updatedTeamData.team_name = team_name || existingTeam.team_name;
    if (stadium !== undefined) updatedTeamData.stadium = stadium || existingTeam.stadium;
    if (coach !== undefined) updatedTeamData.coach = coach || 'Chưa cập nhật';
    if (logo !== undefined) updatedTeamData.logo = logo || 'https://upload.wikimedia.org/wikipedia/commons/thumb/a/ac/No_image_available.svg/300px-No_image_available.svg.png';
    ...
    await logActivity(req, "Cập nhật đội bóng", `Đã cập nhật thông tin đội bóng "${result.team_name}"`);
...
const deleteTeam = async (req, res, next) => {
  ...
  try {
    ...
    await logActivity(req, "Xóa đội bóng", `Đã xóa đội bóng "${teamToDelete.team_name}" khỏi giải đấu`);
...
const getTeamsByIDSeason = async (req, res, next) => {
  try {
    ...
    const teams = await Team.find({ season_id: season_id_obj }).populate("season_id").lean();
    if (teams.length === 0) {
