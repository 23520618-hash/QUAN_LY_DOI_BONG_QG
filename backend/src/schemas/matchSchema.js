...
export const createManualMatchSchema = z.object({
  season_id: z.string().refine((val) => /^[0-9a-fA-F]{24}$/.test(val), { message: "Invalid season ID format" }),
  team1_id: z.string().refine((val) => /^[0-9a-fA-F]{24}$/.test(val), { message: "Invalid team 1 ID format" }),
  team2_id: z.string().refine((val) => /^[0-9a-fA-F]{24}$/.test(val), { message: "Invalid team 2 ID format" }),
  date: z.coerce.date(),
