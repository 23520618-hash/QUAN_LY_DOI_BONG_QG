...
const Rankings = ({ seasonId, token, seasons, formatDate, setSelectedSeasonId, hideDropdown = false }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                let matchesResponse;
                try {
                    matchesResponse = await axios.get(`http://localhost:5000/api/matches/seasons/${seasonId}`, config);
                } catch (err) {
                    if (err.response?.status === 401 && token) {
                        matchesResponse = await axios.get(`http://localhost:5000/api/matches/seasons/${seasonId}`);
                    } else {
                        throw new Error('Không thể lấy danh sách trận đấu: ' + (err.response?.data?.message || err.message));
                    }
                }

                if (matchesResponse.data.status !== 'success' || !Array.isArray(matchesResponse.data.data)) {
                    throw new Error('Dữ liệu trận đấu không hợp lệ.');
                }
                setAllMatches(matchesResponse.data.data);
        ...
        window.addEventListener('matchUpdated', fetchData);
        return () => window.removeEventListener('matchUpdated', fetchData);
    }, [seasonId, token]);
    ...
    const enrichedResults = useMemo(() => {
        ...
        return filteredTeams
            .map(team => {
                ...
                const teamPlayedMatches = allMatches
                    .filter(m => {
                        if (selectedDate) {
                            const selectedDateObj = new Date(selectedDate);
                            const mDate = new Date(m.date);
                            if (mDate > selectedDateObj) return false;
                        }
                        if (!m.score || !/^\d+-\d+$/.test(m.score)) return false;
                        const t1Id = m.team1?._id?.toString() || m.team1?.toString();
                        const t2Id = m.team2?._id?.toString() || m.team2?.toString();
                        return t1Id === team._id || t2Id === team._id;
                    })
                    .sort((a, b) => new Date(b.date) - new Date(a.date));

                const recentMatches = teamPlayedMatches.slice(0, 5);
                const form = recentMatches.map(m => {
                    const parts = m.score.split('-');
                    const score1 = parseInt(parts[0], 10);
                    const score2 = parseInt(parts[1], 10);
                    const t1Id = m.team1?._id?.toString() || m.team1?.toString();
                    const isTeam1 = t1Id === team._id;
                    
                    if (score1 === score2) return 'H';
                    if (isTeam1) {
                        return score1 > score2 ? 'T' : 'B';
                    } else {
                        return score2 > score1 ? 'T' : 'B';
                    }
                }).reverse();
    ...
    }, [teamResults, teams, rankingCriteria, selectedDate, selectedTeam, allMatches]);
                        ...
                        <table className="w-full table-auto border-collapse">
                            <thead>
                                <tr className="bg-gray-900 text-white text-3xl font-bold py-3 px-6 rounded-none border-l-8 border-red-600 mb-6 text-center tracking-wide hover:brightness-110 transition-all duration-200 ">
                                    ...
                                    <th className="px-6 py-4 text-center text-sm font-semibold ">Phong độ</th>
                            ...
                            <tbody>
                                {paginatedResults.map(team => (
                                    <tr key={team.rank} className="border-b border-gray-200 hover:bg-red-50 transition duration-150">
                                        ...
                                        <td className="px-6 py-4 text-center text-gray-700 font-semibold">{team.points}</td>
                                            ...
                                            <div className="flex justify-center gap-1.5">
                                                {team.form && team.form.length > 0 ? (
                                                    team.form.map((result, idx) => {
                                                        ...
                                                        if (result === 'T') { bgColor = 'bg-green-500'; }
                                                        else if (result === 'H') { bgColor = 'bg-yellow-500'; }
                                                        else if (result === 'B') { bgColor = 'bg-red-500'; }
                                                        return (
                                                            <span
                                                                key={idx}
                                                                className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${bgColor} ${textColor} shadow-sm`}
                                                                title={result === 'T' ? 'Thắng' : result === 'H' ? 'Hòa' : 'Thua'}
                                                            >
                                                                {result}
                                                            </span>
                                                        );
