...
const PlayerForm = ({ editingPlayer, setEditingPlayer, setShowForm, setPlayers, token, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const playerDataToSend = {
            ...
            number: formData.number ? formData.number.toString() : '',
            avatar: formData.avatar.trim() || '',
        };

        // Frontend validation
        if (formData.nationality && formData.nationality.toLowerCase().trim() === 'việt nam' && formData.isForeigner) {
            showAlert('Cầu thủ có quốc tịch Việt Nam không thể được đăng ký là cầu thủ ngoại quốc.', 'error');
        ...
        <div className="container mx-auto p-6 bg-white shadow-xl rounded-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                 <fieldset disabled={isSubmitting} className="space-y-6">
                    ...
                    {/* Player Details */}
                    <div>
                        <label htmlFor="name" className="block text-sm font-medium text-gray-700">Tên cầu thủ <span className="text-red-500">*</span>:</label>
                        <input id="name" type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Nhập tên cầu thủ" required className="mt-1 w-full p-3 border border-gray-300 rounded-md focus-glow" />
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div>
                            <label htmlFor="number" className="block text-sm font-medium text-gray-700">Số áo:</label>
                            <input id="number" type="number" value={formData.number} onChange={(e) => setFormData({ ...formData, number: e.target.value })} placeholder="Số áo" min="1" max="999" className="mt-1 w-full p-3 border border-gray-300 rounded-md focus-glow" />
                        </div>
                        <div>
                            <label htmlFor="position" className="block text-sm font-medium text-gray-700">Vị trí:</label>
                            <input id="position" type="text" value={formData.position} onChange={(e) => setFormData({ ...formData, position: e.target.value })} placeholder="Vị trí" className="mt-1 w-full p-3 border border-gray-300 rounded-md focus-glow" />
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label htmlFor="nationality" className="block text-sm font-medium text-gray-700">Quốc tịch <span className="text-red-500">*</span>:</label>
                            <input id="nationality" type="text" value={formData.nationality} onChange={(e) => setFormData({ ...formData, nationality: e.target.value })} placeholder="Quốc tịch" required className="mt-1 w-full p-3 border border-gray-300 rounded-md focus-glow" />
                        </div>
                        <div>
                            <label htmlFor="dob" className="block text-sm font-medium text-gray-700">Ngày sinh <span className="text-red-500">*</span>:</label>
                            <input id="dob" type="date" value={formData.dob} onChange={(e) => setFormData({ ...formData, dob: e.target.value })} required className="mt-1 w-full p-3 border border-gray-300 rounded-md focus-glow" />
                        </div>
                    </div>

                    <div>
                        <label htmlFor="avatar" className="block text-sm font-medium text-gray-700">URL Ảnh đại diện:</label>
                        <input id="avatar" type="text" value={formData.avatar} onChange={(e) => setFormData({ ...formData, avatar: e.target.value })} placeholder="https://example.com/player.jpg hoặc player.png" className="mt-1 w-full p-3 border border-gray-300 rounded-md" />
                    </div>
