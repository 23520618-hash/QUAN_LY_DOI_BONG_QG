// File: truonghoangkhiem/ql_giai_bd_qg_se104_uit/QL_Giai_BD_QG_SE104_UIT-ten-nhanh-moi/frontend/src/components/Matches.jsx
import React, { useEffect, useState, useCallback } from 'react';
import axios from 'axios';
import { CalendarIcon, MapPinIcon, XCircleIcon, ClockIcon, ChevronDownIcon, ChevronUpIcon, UsersIcon } from '@heroicons/react/24/outline';

const Matches = ({
  matches: propMatches,
  setMatches: setPropMatches,
  setEditingMatch = () => { },
  setShowForm = () => { },
  type = 'all',
  onPastMatchesFetched = () => { },
  token,
  seasonId: initialSeasonId,
  limit,
  hideDropdown = false,
}) => {
  const isAdmin = localStorage.getItem('role') === 'admin' || JSON.parse(localStorage.getItem('permissions') || '[]').includes('manage_matches');
  const [localMatches, setLocalMatches] = useState([]);
  const [seasons, setSeasons] = useState([]);
  const [teams, setTeams] = useState([]);
  const [filteredTeamsForDropdown, setFilteredTeamsForDropdown] = useState([]);
  const [selectedSeasonId, setSelectedSeasonId] = useState(() => localStorage.getItem('lastSelectedSeason') || '');
  const [selectedDate, setSelectedDate] = useState('');
  const [selectedTeamId, setSelectedTeamId] = useState('');
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [activeTab, setActiveTab] = useState('upcoming');
  const [currentPage, setCurrentPage] = useState(1);
  const matchesPerPage = 7;

  // State đội hình
  const [expandedMatchId, setExpandedMatchId] = useState(null);
  const [matchLineups, setMatchLineups] = useState({ team1: null, team2: null });
  const [lineupLoading, setLineupLoading] = useState(false);
  const [lineupError, setLineupError] = useState('');

  const [deletingId, setDeletingId] = useState(null);

  // Không cần allPlayersMap nữa
  // const [allPlayersMap, setAllPlayersMap] = useState({});

  const matchesToDisplayState = propMatches !== undefined ? propMatches : localMatches;
  const setMatchesState = setPropMatches || setLocalMatches;

  const defaultLogoUrl = 'https://th.bing.com/th/id/OIP.iiLfIvv8F-PfjMrjObypGgHaHa?rs=1&pid=ImgDetMain';

  const handleImageError = (e) => {
    e.target.src = defaultLogoUrl;
  };

  const isValidDateString = (dateString) => {
    if (!dateString) return false;
    const date = new Date(dateString);
    return date instanceof Date && !isNaN(date);
  };

  const formatDateToDisplay = (dateString) => {
    if (!isValidDateString(dateString)) return 'Chưa rõ';
    const date = new Date(dateString);
    return date.toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' });
  };

  const formatTimeToDisplay = (dateString) => {
    if (!isValidDateString(dateString)) return 'N/A';
    const date = new Date(dateString);
    if (date.getUTCHours() === 0 && date.getUTCMinutes() === 0 && date.getUTCSeconds() === 0 && date.getUTCMilliseconds() === 0 && !dateString.includes('T') && !dateString.includes(':')) {
      return "N/A";
    }
    return date.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false });
  };

  // useEffect để lấy allPlayers đã được loại bỏ vì không cần thiết nữa

  useEffect(() => {
    if (hideDropdown) return;
    const fetchSeasons = async () => {
      setLoading(true);
      try {
        const response = await axios.get('http://localhost:5000/api/seasons');
        const seasonsData = response.data.data || [];
        setSeasons(seasonsData);
        const storedSeasonId = localStorage.getItem('lastSelectedSeason');
        const storedSeasonIsValid = storedSeasonId && seasonsData.some(s => s._id === storedSeasonId);

        if (initialSeasonId) {
            setSelectedSeasonId(initialSeasonId);
        } else if (storedSeasonIsValid) {
            setSelectedSeasonId(storedSeasonId);
        } else if (seasonsData.length > 0) {
            const defaultSeason = seasonsData[0]._id;
            setSelectedSeasonId(defaultSeason);
            localStorage.setItem('lastSelectedSeason', defaultSeason);
        }
      } catch (err) {
        console.error('Lỗi lấy mùa giải:', err);
        setError('Không thể tải danh sách mùa giải');
      }
    };
    fetchSeasons();
  }, [hideDropdown, initialSeasonId]);

  useEffect(() => {
    if (hideDropdown) return;
    const fetchTeams = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/teams');
        setTeams(response.data.data || []);
      } catch (err) {
        console.error('Lỗi lấy đội bóng:', err);
      }
    };
    fetchTeams();
  }, [hideDropdown]);

  useEffect(() => {
    if (hideDropdown) return;
    if (selectedSeasonId) {
      const filtered = teams.filter((team) => team.season_id && (team.season_id._id || team.season_id).toString() === selectedSeasonId);
      setFilteredTeamsForDropdown(filtered);
      if (selectedTeamId && !filtered.some(t => t._id === selectedTeamId)) {
        setSelectedTeamId('');
      }
    } else {
      setFilteredTeamsForDropdown(teams);
    }
  }, [selectedSeasonId, teams, hideDropdown, selectedTeamId]);


  useEffect(() => {
    const fetchMatches = async () => {
      const seasonToFetch = selectedSeasonId || initialSeasonId;
      if (!seasonToFetch && !selectedTeamId && !selectedDate && !hideDropdown) {
        setMatchesState([]);
        setLoading(false);
        if (!hideDropdown) setError("Vui lòng chọn một mùa giải.");
        return;
      }
      setLoading(true);
      setError('');
      let url;
      if (selectedTeamId && seasonToFetch) {
        url = `http://localhost:5000/api/matches/teams/${selectedTeamId}`;
      } else if (selectedDate && seasonToFetch) {
        url = `http://localhost:5000/api/matches/seasons/${seasonToFetch}/${selectedDate}`;
      } else if (seasonToFetch) {
        url = `http://localhost:5000/api/matches/seasons/${seasonToFetch}`;
      } else if (!hideDropdown) {
        setMatchesState([]);
        setLoading(false);
        setError("Vui lòng chọn một mùa giải hoặc bộ lọc khác.");
        return;
      } else {
        if (!initialSeasonId) {
          setMatchesState([]);
          setLoading(false);
          return;
        }
        url = `http://localhost:5000/api/matches/seasons/${initialSeasonId}`;
      }

      try {
        const response = await axios.get(url);
        let matchesData = response.data.data || [];
        if (selectedTeamId && seasonToFetch && url.includes(`/teams/${selectedTeamId}`)) {
          matchesData = matchesData.filter(match => match.season_id && (match.season_id._id || match.season_id).toString() === seasonToFetch);
        }
        setMatchesState(matchesData);
        if (type === 'home' && typeof onPastMatchesFetched === 'function') {
          const now = new Date();
          const pastAndPlayed = matchesData
            .filter((match) => {
              const matchDateTime = new Date(match.date);
              return matchDateTime <= now && (match.score !== null && match.score !== "" && /^\d+-\d+$/.test(match.score));
            })
            .sort((a, b) => new Date(b.date) - new Date(a.date))
            .slice(0, limit || 5);
          onPastMatchesFetched(pastAndPlayed);
        }
      } catch (err) {
        console.error('Lỗi lấy trận đấu:', err.response?.data || err.message);
        setMatchesState([]);
        if (err.response?.status === 404) {
          setError('Không có trận đấu nào phù hợp với lựa chọn của bạn.');
        } else {
          setError('Không thể tải danh sách trận đấu. Vui lòng thử lại.');
        }
      } finally {
        setLoading(false);
      }
    };

    if ((selectedSeasonId || initialSeasonId) || (!hideDropdown && (selectedDate || selectedTeamId))) {
      fetchMatches();
    } else if (!hideDropdown && seasons.length > 0 && !selectedSeasonId && !initialSeasonId) {
      setLoading(false);
      setMatchesState([]);
    } else if (!hideDropdown && seasons.length === 0 && !initialSeasonId) {
      setLoading(false);
      setMatchesState([]);
    }
  }, [selectedSeasonId, selectedDate, selectedTeamId, initialSeasonId, hideDropdown, type, limit, onPastMatchesFetched, setMatchesState, seasons]);


  const handleEdit = (match) => {
    setEditingMatch(match);
    setShowForm(true);
    setExpandedMatchId(null);
  };

  const handleDelete = async (id) => {
    if (!token) {
      setError('Vui lòng đăng nhập để xóa trận đấu');
      return;
    }
    if (window.confirm('Bạn có chắc chắn muốn xóa trận đấu này? Điều này sẽ xóa luôn đội hình và ảnh hưởng kết quả, xếp hạng liên quan.')) {
      setDeletingId(id);
      try {
        await axios.delete(`http://localhost:5000/api/matches/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setMatchesState((prevMatches) => prevMatches.filter((match) => (match.id || match._id) !== id));
      } catch (err) {
        console.error("Delete error:", err.response?.data || err.message);
        setError(err.response?.data?.message || 'Không thể xóa trận đấu.');
      } finally {
        setDeletingId(null);
      }
    }
  };

  const handleToggleLineups = async (match) => {
    const matchId = match.id || match._id;
    if (expandedMatchId === matchId) {
      setExpandedMatchId(null);
      setMatchLineups({ team1: null, team2: null });
    } else {
      setExpandedMatchId(matchId);
      setLineupLoading(true);
      setLineupError('');
      try {
        const [lineup1Res, lineup2Res] = await Promise.all([
          axios.get(`http://localhost:5000/api/matchlineups/match/${matchId}/team/${match.team1._id}`),
          axios.get(`http://localhost:5000/api/matchlineups/match/${matchId}/team/${match.team2._id}`)
        ]);
        setMatchLineups({
          team1: lineup1Res.data.data,
          team2: lineup2Res.data.data
        });
      } catch (err) {
        console.error("Lỗi lấy đội hình:", err.response?.data || err.message);
        if (err.response?.status === 404) {
          setLineupError('Không tìm thấy đội hình cho trận đấu này.');
        } else {
          setLineupError('Không thể tải đội hình.');
        }
        setMatchLineups({ team1: null, team2: null });
      } finally {
        setLineupLoading(false);
      }
    }
  };


  const handleDateChange = useCallback((value) => {
    setSelectedDate(value);
    setCurrentPage(1);
    setError('');
    setExpandedMatchId(null);
  }, []);

  const handleTeamChange = useCallback((value) => {
    setSelectedTeamId(value);
    setCurrentPage(1);
    setError('');
    setExpandedMatchId(null);
  }, []);

  const handleSeasonChange = useCallback((value) => {
    setSelectedSeasonId(value);
    if (value) {
      localStorage.setItem('lastSelectedSeason', value);
    } else {
      localStorage.removeItem('lastSelectedSeason');
    }
    setSelectedDate('');
    setSelectedTeamId('');
    setCurrentPage(1);
    setError('');
    setExpandedMatchId(null);
  }, []);

  const handleResetFilter = useCallback(() => {
    setSelectedDate('');
    setSelectedTeamId('');
    setCurrentPage(1);
    setError('');
    setExpandedMatchId(null);
  }, []);

  const now = new Date();

  const pastMatches = matchesToDisplayState
    .filter((match) => new Date(match.date) <= now)
    .sort((a, b) => new Date(b.date) - new Date(a.date));

  const upcomingMatches = matchesToDisplayState
    .filter((match) => new Date(match.date) > now)
    .sort((a, b) => new Date(a.date) - new Date(b.date));

  const indexOfLastMatch = currentPage * matchesPerPage;
  const indexOfFirstMatch = indexOfLastMatch - matchesPerPage;

  const currentMatchesToDisplayList = activeTab === 'upcoming'
    ? upcomingMatches.slice(indexOfFirstMatch, indexOfLastMatch)
    : pastMatches.slice(indexOfFirstMatch, indexOfLastMatch);

  const totalPages = activeTab === 'upcoming'
    ? Math.ceil(upcomingMatches.length / matchesPerPage) || 1
    : Math.ceil(pastMatches.length / matchesPerPage) || 1;

  const handleNextPage = useCallback(() => {
    if (currentPage < totalPages) {
      setCurrentPage((prev) => prev + 1);
    }
  }, [currentPage, totalPages]);

  const handlePrevPage = useCallback(() => {
    if (currentPage > 1) {
      setCurrentPage((prev) => prev - 1);
    }
  }, [currentPage]);

  const renderMatchScore = (score, matchDateStr) => {
    const matchDateTime = new Date(matchDateStr);
    if (score !== null && score !== "" && /^\d+-\d+$/.test(score)) {
      return (
        <div className="bg-red-50/80 backdrop-blur-sm px-3.5 py-1 rounded-full border border-red-100/50 shadow-sm animate-score-reveal">
          <span className="text-base font-bold text-red-600 tracking-wider font-heading">{score}</span>
        </div>
      );
    } else if (matchDateTime <= now) {
      return <span className="text-xs text-gray-500 italic px-2 py-0.5 bg-gray-100 rounded">Chưa có tỉ số</span>;
    } else {
      return (
        <div className="bg-gray-100/80 px-4 py-1 rounded-full text-sm font-bold text-gray-500 border border-gray-200/50 font-heading tracking-widest uppercase">
          X - X
        </div>
      );
    }
  };

  const renderLineupSection = (teamLineup, teamName) => {
    if (!teamLineup || !teamLineup.players || teamLineup.players.length === 0) {
      return <p className="text-sm text-gray-500 italic">Đội hình của {teamName} chưa được cập nhật.</p>;
    }
    return (
      <div className="w-full md:w-1/2 p-2">
        <h4 className="text-md font-semibold text-gray-800 mb-2 font-heading flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-red-600"></span> {teamName}
        </h4>
        <ul className="list-none pl-0 space-y-1 text-sm text-gray-600 max-h-48 overflow-y-auto pr-1">
          {teamLineup.players.map((p) => (
            <li key={p.player_id?._id || p.player_id} className="flex items-center gap-2 py-1 px-2 hover:bg-gray-100 rounded-lg transition-colors">
              <span className="font-semibold text-red-600 w-6">#{p.jersey_number}</span>
              <span className="text-gray-700 flex-grow">{p.player_id?.name || 'Không rõ tên'}</span>
              <span className="text-xs text-gray-400 bg-gray-200/60 px-1.5 py-0.5 rounded font-medium">{p.position}</span>
            </li>
          ))}
        </ul>
      </div>
    );
  };

  const renderMatchesList = (list) => (
    list.length > 0 ? (
      list.map((match, i) => (
        <div key={match.id || match._id}
          className="border-b border-gray-100 w-full stagger-item hover:bg-gray-50/50 transition-colors duration-200 rounded-xl px-2 my-1.5"
          style={{ animationDelay: `${i * 60}ms` }}
        >
          <div className="grid grid-cols-1 md:grid-cols-12 items-center py-4 gap-4 px-2 md:px-4">
            {/* Team 1 */}
            <div className="col-span-12 md:col-span-5 flex items-center gap-3 group">
              <div className="w-10 h-10 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden shadow-sm group-hover:scale-105 transition-transform duration-300">
                <img
                  src={match.team1?.logo ? (match.team1.logo.startsWith('http') || match.team1.logo.startsWith('/') ? match.team1.logo : `/images/teams_logo/${match.team1.logo}`) : defaultLogoUrl}
                  alt={`${match.team1?.team_name || 'Đội 1'} logo`}
                  className="w-8 h-8 object-contain"
                  onError={handleImageError}
                />
              </div>
              <span className="text-gray-800 font-semibold truncate group-hover:text-red-700 transition-colors" title={match.team1?.team_name || 'Đội không xác định'}>
                {match.team1?.team_name || 'Đội không xác định'}
              </span>
            </div>
            {/* Score & Info */}
            <div className="col-span-12 md:col-span-2 flex flex-col items-center text-center my-2 md:my-0 space-y-1">
              {renderMatchScore(match.score, match.date)}
              <div className="text-xs text-gray-400 font-medium space-y-0.5">
                <div className="flex items-center justify-center gap-1">
                  <CalendarIcon className="h-3.5 w-3.5 text-gray-400" />
                  <span>{formatDateToDisplay(match.date)}</span>
                </div>
                {formatTimeToDisplay(match.date) !== "N/A" && (
                  <div className="flex items-center justify-center gap-1">
                    <ClockIcon className="h-3.5 w-3.5 text-gray-400" />
                    <span>{formatTimeToDisplay(match.date)}</span>
                  </div>
                )}
              </div>
              <span className="text-xs text-gray-400 truncate max-w-full px-1 flex items-center justify-center gap-1" title={match.stadium || 'Chưa rõ sân'}>
                <MapPinIcon className="h-3.5 w-3.5 text-gray-400 flex-shrink-0" />
                <span className="truncate">{match.stadium || 'Chưa rõ sân'}</span>
              </span>
            </div>
            {/* Team 2 */}
            <div className="col-span-12 md:col-span-5 flex items-center justify-start md:justify-end gap-3 group">
              <span className="text-gray-800 font-semibold text-left md:text-right truncate order-2 md:order-1 group-hover:text-red-700 transition-colors" title={match.team2?.team_name || 'Đội không xác định'}>
                {match.team2?.team_name || 'Đội không xác định'}
              </span>
              <div className="w-10 h-10 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden shadow-sm order-1 md:order-2 group-hover:scale-105 transition-transform duration-300">
                <img
                  src={match.team2?.logo ? (match.team2.logo.startsWith('http') || match.team2.logo.startsWith('/') ? match.team2.logo : `/images/teams_logo/${match.team2.logo}`) : defaultLogoUrl}
                  alt={`${match.team2?.team_name || 'Đội 2'} logo`}
                  className="w-8 h-8 object-contain"
                  onError={handleImageError}
                />
              </div>
            </div>

            {/* Goal Scorers */}
            {new Date(match.date) <= now && match.score && /^\d+-\d+$/.test(match.score) && match.goalDetails && match.goalDetails.length > 0 && (
              <div className="col-span-12 mt-3 pt-3 border-t border-gray-100">
                <p className="font-semibold text-xs text-gray-400 uppercase tracking-wider mb-2 text-center font-heading">Ghi bàn</p>
                <div className="flex flex-col md:flex-row justify-between gap-4 text-xs text-gray-600">
                  {/* Column for Team 1 Scorers */}
                  <div className="w-full md:w-1/2">
                    <h4 className="font-semibold text-gray-800 text-center md:text-left mb-1.5 font-heading">{match.team1?.team_name || 'Đội 1'}</h4>
                    <ul className="list-none pl-0 space-y-1">
                      {match.goalDetails
                        .filter(goal => goal.team_id === match.team1?._id)
                        .map((goal, index) => {
                          const scorerName = goal.player_id?.name || 'N/A';
                          return (
                            <li key={`t1-goal-${index}-${match.id || match._id}`} className="text-gray-600 flex items-center gap-1.5 justify-center md:justify-start">
                              <span>⚽</span>
                              <span>{scorerName}</span>
                              <span className="font-semibold text-gray-400">({goal.minute}')</span>
                              {goal.goalType === 'OG' && <span className="text-red-500 font-medium"> (OG)</span>}
                              {goal.goalType !== 'normal' && goal.goalType !== 'OG' && ` (${goal.goalType.toUpperCase()})`}
                            </li>
                          );
                        })}
                      {match.goalDetails.filter(goal => goal.team_id === match.team1?._id).length === 0 && (
                        <li className="italic text-gray-400 text-center md:text-left">Không có bàn thắng</li>
                      )}
                    </ul>
                  </div>

                  <div className="hidden md:block border-l border-gray-100 mx-2"></div>

                  {/* Column for Team 2 Scorers */}
                  <div className="w-full md:w-1/2">
                    <h4 className="font-semibold text-gray-800 text-center md:text-right mb-1.5 font-heading">{match.team2?.team_name || 'Đội 2'}</h4>
                    <ul className="list-none pl-0 space-y-1 md:text-right">
                      {match.goalDetails
                        .filter(goal => goal.team_id === match.team2?._id)
                        .map((goal, index) => {
                          const scorerName = goal.player_id?.name || 'N/A';
                          return (
                            <li key={`t2-goal-${index}-${match.id || match._id}`} className="text-gray-600 flex items-center gap-1.5 justify-center md:justify-end">
                              <span>⚽</span>
                              <span>{scorerName}</span>
                              <span className="font-semibold text-gray-400">({goal.minute}')</span>
                              {goal.goalType === 'OG' && <span className="text-red-500 font-medium"> (OG)</span>}
                              {goal.goalType !== 'normal' && goal.goalType !== 'OG' && ` (${goal.goalType.toUpperCase()})`}
                            </li>
                          );
                        })}
                      {match.goalDetails.filter(goal => goal.team_id === match.team2?._id).length === 0 && (
                        <li className="italic text-gray-400 text-center md:text-right">Không có bàn thắng</li>
                      )}
                    </ul>
                  </div>
                </div>
              </div>
            )}

            {/* Action Buttons */}
            <div className="col-span-12 flex justify-center items-center gap-2 mt-2">
              <button
                onClick={() => handleToggleLineups(match)}
                className="bg-indigo-50/80 text-indigo-600 px-3 py-1.5 rounded-xl hover:bg-indigo-100 transition-all text-xs font-semibold flex items-center border border-indigo-100"
              >
                <UsersIcon className="h-4 w-4 mr-1" />
                Đội Hình {expandedMatchId === (match.id || match._id) ? <ChevronUpIcon className="h-4 w-4 ml-1" /> : <ChevronDownIcon className="h-4 w-4 ml-1" />}
              </button>
              {token && isAdmin && !hideDropdown && (
                <>
                  <button
                    onClick={() => handleEdit(match)}
                    className="bg-blue-600 text-white px-3 py-1.5 rounded-xl hover:bg-blue-700 hover-glow transition-all text-xs font-semibold shadow-sm"
                  >
                    Sửa
                  </button>
                  {deletingId === (match.id || match._id) ? (
                    <div className="flex justify-center items-center w-14 h-7">
                        <div className="animate-spin rounded-full h-4 w-4 border-2 border-red-600 border-t-transparent"></div>
                    </div>
                  ) : (
                    <button
                      onClick={() => handleDelete(match.id || match._id)}
                      className="bg-red-50 text-red-600 px-3 py-1.5 rounded-xl hover:bg-red-100 transition-all text-xs font-semibold border border-red-100"
                      disabled={deletingId !== null}
                    >
                      Xóa
                    </button>
                  )}
                </>
              )}
            </div>
          </div>
          {/* Lineup Display */}
          {expandedMatchId === (match.id || match._id) && (
            <div className="px-4 md:px-6 py-4 bg-gray-50/50 backdrop-blur-sm rounded-xl border border-gray-100 my-2">
              {lineupLoading && <div className="flex justify-center py-2"><div className="animate-spin rounded-full h-6 w-6 border-2 border-red-600 border-t-transparent"></div></div>}
              {lineupError && <p className="text-sm text-red-500 text-center font-medium">{lineupError}</p>}
              {!lineupLoading && !lineupError && (
                <div className="flex flex-col md:flex-row justify-between gap-4">
                  {renderLineupSection(matchLineups.team1, match.team1?.team_name || 'Đội 1')}
                  {renderLineupSection(matchLineups.team2, match.team2?.team_name || 'Đội 2')}
                </div>
              )}
            </div>
          )}
        </div>
      ))
    ) : (
      <div className="text-center py-10">
        <span className="text-4xl block mb-2">⚽</span>
        <p className="text-gray-500 text-sm font-medium">
          {error && !loading ? error : (activeTab === 'upcoming' ? 'Không có trận đấu nào sắp diễn ra.' : 'Không có trận đấu nào đã diễn ra.')}
        </p>
      </div>
    )
  );

  if (loading && !hideDropdown && (!selectedSeasonId && !initialSeasonId)) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="animate-spin rounded-full h-10 w-10 border-4 border-red-200 border-t-red-600"></div>
      </div>
    );
  }

  return (
    <div className={`w-full ${!hideDropdown ? 'flex flex-col lg:flex-row gap-6' : ''}`}>
      {!hideDropdown && (
        <div className="w-full lg:w-72 flex-shrink-0 animate-scale-in">
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-100 shadow-sm p-5 md:p-6">
            <h2 className="text-lg md:text-xl font-bold text-gray-800 mb-4 font-heading flex items-center gap-1.5">
              <span>🔍</span> Bộ lọc trận đấu
            </h2>
            {(seasons.length === 0 && !loading && !initialSeasonId) && <p className="text-gray-400 text-xs italic">Không có mùa giải nào để lọc.</p>}
            {(seasons.length > 0 || initialSeasonId) && (
              <div className="flex flex-col gap-4">
                {!initialSeasonId &&
                  <div>
                    <label htmlFor="season-select" className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Mùa giải</label>
                    <select
                      id="season-select"
                      value={selectedSeasonId}
                      onChange={(e) => handleSeasonChange(e.target.value)}
                      className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2.5 text-gray-700 font-medium focus:ring-2 focus:ring-red-500 focus-glow transition-all duration-200 text-sm shadow-sm"
                    >
                      <option value="">Chọn mùa giải</option>
                      {seasons.map((season) => (
                        <option key={season._id} value={season._id}>
                          {season.season_name}
                        </option>
                      ))}
                    </select>
                  </div>
                }
                {type === 'all' && (
                  <>
                    <div>
                      <label htmlFor="match-date" className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Ngày thi đấu</label>
                      <div className="relative">
                        <CalendarIcon className="absolute left-3.5 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
                        <input
                          type="date"
                          id="match-date"
                          value={selectedDate}
                          onChange={(e) => handleDateChange(e.target.value)}
                          className="w-full pl-10 bg-white border border-gray-300 rounded-xl px-3 py-2.5 text-gray-700 font-medium focus:ring-2 focus:ring-red-500 focus-glow transition-all duration-200 text-sm shadow-sm"
                          disabled={!selectedSeasonId && !initialSeasonId}
                        />
                      </div>
                    </div>
                    <div>
                      <label htmlFor="team-select" className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Đội bóng</label>
                      <select
                        id="team-select"
                        value={selectedTeamId}
                        onChange={(e) => handleTeamChange(e.target.value)}
                        className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2.5 text-gray-700 font-medium focus:ring-2 focus:ring-red-500 focus-glow transition-all duration-200 text-sm shadow-sm"
                        disabled={(!selectedSeasonId && !initialSeasonId) || filteredTeamsForDropdown.length === 0}
                      >
                        <option value="">Tất cả đội bóng</option>
                        {filteredTeamsForDropdown.map((team) => (
                          <option key={team._id} value={team._id}>
                            {team.team_name}
                          </option>
                        ))}
                      </select>
                    </div>
                  </>
                )}
                {type === 'all' && (selectedDate || selectedTeamId) && (
                  <div>
                    <button
                      onClick={handleResetFilter}
                      className="flex items-center gap-2 bg-gray-100 hover:bg-gray-250 text-gray-600 px-3 py-2.5 rounded-xl hover:bg-gray-200 transition-colors duration-200 w-full justify-center text-xs font-semibold border border-gray-200"
                    >
                      <XCircleIcon className="w-4 h-4" />
                      Xóa bộ lọc
                    </button>
                  </div>
                )}
              </div>
            )}
          </div>
        </div>
      )}
      <div className="flex-grow min-w-0">
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-100 shadow-sm p-4 md:p-6">
          {error && !loading && (
            <div className="text-center p-3 mb-4 bg-red-50 text-red-600 border border-red-100 rounded-xl font-medium text-xs">
              {error}
            </div>
          )}
          <div className="flex border-b border-gray-100 mb-4 md:mb-6">
            <button
              className={`px-4 py-2.5 text-sm md:text-base font-bold font-heading transition-all ${activeTab === 'upcoming' ? 'border-b-2 border-red-600 text-red-600' : 'text-gray-400 hover:text-gray-600'}`}
              onClick={() => { setActiveTab('upcoming'); setCurrentPage(1); setExpandedMatchId(null); }}
            > Sắp diễn ra ({upcomingMatches.length}) </button>
            <button
              className={`px-4 py-2.5 text-sm md:text-base font-bold font-heading transition-all ${activeTab === 'past' ? 'border-b-2 border-red-600 text-red-600' : 'text-gray-400 hover:text-gray-600'}`}
              onClick={() => { setActiveTab('past'); setCurrentPage(1); setExpandedMatchId(null); }}
            > Đã diễn ra ({pastMatches.length}) </button>
          </div>
          {(loading && (selectedSeasonId || initialSeasonId)) && (
            <div className="flex justify-center items-center py-8">
              <div className="animate-spin rounded-full h-8 w-8 border-2 border-red-600 border-t-transparent"></div>
            </div>
          )}
          <div className="min-h-[200px]">
            {!loading && renderMatchesList(currentMatchesToDisplayList)}
          </div>
          {(totalPages > 1 && currentMatchesToDisplayList.length > 0) && (
            <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-100 gap-2">
              <button
                onClick={handlePrevPage}
                disabled={currentPage === 1}
                className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all ${currentPage === 1 ? 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200' : 'bg-red-600 text-white hover:bg-red-700 hover-glow shadow-sm'}`}
              > Trang trước </button>
              <span className="text-gray-500 text-xs font-semibold"> Trang {currentPage} / {totalPages} </span>
              <button
                onClick={handleNextPage}
                disabled={currentPage >= totalPages}
                className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all ${currentPage >= totalPages ? 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200' : 'bg-red-600 text-white hover:bg-red-700 hover-glow shadow-sm'}`}
              > Trang sau </button>
            </div>
          )}
        </div>
      </div>
    </div>
  );
};

export default Matches;