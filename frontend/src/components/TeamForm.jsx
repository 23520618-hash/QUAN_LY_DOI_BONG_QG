        ...
        <div className="container mx-auto p-4 max-w-lg bg-white rounded-lg shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                 <fieldset disabled={isSubmitting} className="space-y-4">
                    <div>
                        <label htmlFor="season_id" className="block mb-1 font-medium text-gray-700">Mùa giải <span className="text-red-500">*</span>:</label>
                        <select
                            id="season_id"
                    ...
                    <div>
                        <label className="block mb-1 font-medium text-gray-700">Tên đội <span className="text-red-500">*</span>:</label>
                        <input type="text" value={formData.team_name} onChange={(e) => setFormData({ ...formData, team_name: e.target.value })} placeholder="Tên đội" className="w-full p-2 border border-gray-300 rounded-md focus-glow" required />
                    </div>
                    <div>
                        <label className="block mb-1 font-medium text-gray-700">Sân vận động <span className="text-red-500">*</span>:</label>
                        <input type="text" value={formData.stadium} onChange={(e) => setFormData({ ...formData, stadium: e.target.value })} placeholder="Sân vận động" className="w-full p-2 border border-gray-300 rounded-md focus-glow" required />
                    </div>
                    <div>
                        <label className="block mb-1 font-medium text-gray-700">Huấn luyện viên:</label>
                        <input type="text" value={formData.coach} onChange={(e) => setFormData({ ...formData, coach: e.target.value })} placeholder="Huấn luyện viên" className="w-full p-2 border border-gray-300 rounded-md focus-glow" />
                    </div>
                    <div>
                        <label className="block mb-1 font-medium text-gray-700">URL logo:</label>
                        <input type="text" value={formData.logo} onChange={(e) => setFormData({ ...formData, logo: e.target.value })} placeholder="Link URL hoặc tên file (vd: logo.png)" className="w-full p-2 border border-gray-300 rounded-md focus-glow" />
                    </div>
