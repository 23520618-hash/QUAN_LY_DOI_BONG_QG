        ...
        try {
            ...
            if (editingRegulation) {
                await axios.put(
                    ...
                    { season_id: formData.season_id, regulation_name: formData.regulation_name, rules },
                    { headers }
                );
            } else {
            ...
            if (err.response?.status === 409) {
                const conflicts = err.response.data.data;
                const conflictReasons = conflicts.map(c => c.reason).join('\\n');
                setIsSubmitting(false);
                showConfirm(`Quy định mới sẽ gây xung đột với dữ liệu hiện tại:\\n${conflictReasons}\\n\\nBạn có chắc chắn muốn ép lưu quy định và đánh dấu dữ liệu bị xung đột không?`, async () => {
                    try {
                        setIsSubmitting(true);
                        let rules = {}; // rules is already in outer scope but let's just pass data
                        const dataForRetry = {
                            season_id: formData.season_id,
                            regulation_name: formData.regulation_name,
                            rules: err.response.config.data ? JSON.parse(err.response.config.data).rules : {}
                        };
                        const headers = { Authorization: `Bearer ${token}` };
                        if (editingRegulation) {
                            await axios.put(
                                `http://localhost:5000/api/regulations/${editingRegulation._id}`,
                                { season_id: formData.season_id, regulation_name: formData.regulation_name, rules: dataForRetry.rules, force: true },
                                { headers }
                            );
                        } else {
                            await axios.post('http://localhost:5000/api/regulations/', { ...dataForRetry, force: true }, { headers });
                        }
                        onSuccess();
                    } catch (retryErr) {
                        setError(retryErr.response?.data?.message || retryErr.message || 'Không thể lưu quy định');
                    } finally {
                        setIsSubmitting(false);
                    }
                });
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="Age Regulation">Quy định tuổi</option>
                        <option value="Match Rules">Quy định trận đấu</option>
                        <option value="Goal Rules">Quy định bàn thắng</option>
                        <option value="Ranking Rules">Quy định xếp hạng</option>
                    </select>
                </div>
