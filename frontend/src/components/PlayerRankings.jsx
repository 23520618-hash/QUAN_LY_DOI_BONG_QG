...
const PlayerRankings = ({ seasonId, token }) => {
    ...
    useEffect(() => {
        ...
        window.addEventListener('matchUpdated', fetchRankings);
        return () => window.removeEventListener('matchUpdated', fetchRankings);
    }, [seasonId]);

    const handleDelete = async (id) => {
        if (!token) {
            showAlert('Vui lòng đăng nhập để xóa xếp hạng.', 'warning');
            return;
        }

        showConfirm('Bạn có chắc chắn muốn xóa dữ liệu xếp hạng này không?', async () => {
            try {
            ...
            setRankings(rankings.filter((ranking) => ranking._id !== id));
                ...
                showAlert('Không thể xóa xếp hạng cầu thủ', 'error');
            }
        });
    };
                                ...
                                >
                                    ...
                                    <td className="w-64 py-4 px-8 border-b-2 border-gray-300 text-lg text-gray-800 flex items-center">
                                        <div className="flex items-center space-x-3 min-w-0">
                                            {ranking.avatar ? (
                                                <img
                                                    src={ranking.avatar.startsWith('http') || ranking.avatar.startsWith('/') ? ranking.avatar : `/images/players_avatar/${ranking.avatar}`}
