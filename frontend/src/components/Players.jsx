...
const Players = ({ setEditingPlayer, setShowForm, token, players, setError, handleSuccess, loading, setAllPlayers }) => {
    ...
    const handleKeepConflict = async (player) => {
        try {
            await axios.put(`${API_URL}/api/players/${player._id}`, { is_conflicting: false, conflict_reason: '' }, {
                headers: { Authorization: `Bearer ${token}` },
            });
            setAllPlayers((prevPlayers) => 
                prevPlayers.map((p) => p._id === player._id ? { ...p, is_conflicting: false, conflict_reason: '' } : p)
            );
            handleSuccess('Đã giữ lại thông tin cầu thủ.');
        } catch (err) {
            setError('Lỗi khi giữ lại thông tin cầu thủ: ' + (err.response?.data?.message || err.message));
        }
    };
                    ...
                    {Object.values(groupedPlayers).map(({ team, players }) => (
                        <div key={team._id} className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-lg p-6 shadow-md">
                            <div className="flex items-center mb-4 pb-3 border-b border-gray-300">
                                <img src={team.logo ? (team.logo.startsWith('http') || team.logo.startsWith('/') ? team.logo : `/images/teams_logo/${team.logo}`) : ''} alt={`${team.team_name} logo`} className="w-12 h-12 rounded-full object-contain mr-4 border border-gray-200" onError={(e) => (e.target.src = 'https://th.bing.com/th/id/OIP.dSoxOf16Bt30Ntp4xXxg6gAAAA?rs=1&pid=ImgDetMain')} />
                                <h3 className="text-2xl font-semibold text-gray-800">{team.team_name}</h3>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                {players.map((player) => {
                                    if (player.is_conflicting && (!token || !isAdmin)) return null;
                                    return (
                                    <div key={player._id} className={`bg-white hover:shadow-xl transition-shadow duration-300 ease-in-out border border-gray-200 rounded-lg p-4 shadow-sm flex flex-col justify-between relative ${player.is_conflicting ? 'opacity-50 ring-2 ring-red-500' : ''}`}>
                                        {player.is_conflicting && isAdmin && (
                                            <div className="absolute top-0 left-0 bg-red-100 text-red-600 px-2 py-1 text-[10px] font-bold rounded-br-lg z-10 w-full text-center">
                                                Xung đột: {player.conflict_reason}
                                            </div>
                                        )}
                                        <div className="flex-grow mt-4">
                                            <div className="flex flex-col items-center text-center">
                                                <img src={player.avatar ? (player.avatar.startsWith('http') || player.avatar.startsWith('/') ? player.avatar : `/images/players_avatar/${player.avatar}`) : defaultPlayerAvatar} alt={`${player.name}`} className="w-24 h-24 rounded-full object-cover mb-3 shadow-md" onError={handleImageError} />
                                        ...
                                        {token && isAdmin && (
                                            <div className="mt-4 flex space-x-2 justify-center pt-3 border-t border-gray-200 flex-wrap gap-y-2">
                                                {player.is_conflicting && (
                                                    <button onClick={() => handleKeepConflict(player)} className="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded-md shadow-sm text-xs transition-colors w-full mb-1">
                                                        Giữ lại
                                                    </button>
                                                )}
